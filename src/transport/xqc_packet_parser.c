...
xqc_packet_parse_cid(xqc_cid_t *dcid, xqc_cid_t *scid, uint8_t cid_len, const unsigned char *buf, size_t size)
{
    ...
    if ((buf[0] & 0x40) == 0 && (buf[0] & 0x80) == 0) {/* shouldn't it be if (buf[0] & 0x40) == 0), ill packet only if fix bit is not 0 ?  . question by carr001 */
        return -XQC_EILLPKT;
    }
...
xqc_packet_parse_short_header(xqc_connection_t *c, xqc_packet_in_t *packet_in)
{
    ...
    /* check fixed bit(0x40) = 1 */  // 0100 0000
    if ((pos[0] & 0x40) == 0) {
        xqc_log(c->log, XQC_LOG_ERROR, "|parse short header: fixed bit err|pos[0]:%d", (uint32_t)pos[0]);
        return -XQC_EILLPKT;
    }

    xqc_uint_t spin_bit = (pos[0] & 0x20) >> 5; // 0010 0000  spin  left shift 5 bits to get one bit --carr001
    pos += 1;
